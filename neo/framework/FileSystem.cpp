...
public:
	...
	virtual int				AddResourceFile( const char * resourceFileName );
	virtual void			RegisterModSearchPath( const char * gameDir );
...
const char *idFileSystemLocal::BuildOSPath( const char *base, const char *game, const char *relativePath ) {
	...
	// Fixed real bug (direct user report): RegisterModSearchPath's `gameDir`
	// param is documented/intended as a folder name RELATIVE to fs_basepath
	// (e.g. "map_pack/base", mirroring the built-in "base"/"d3xp" dirs) --
	// but a user who instead pastes a full absolute path (e.g.
	// "C:\DOOM-3-BFG\map_pack\base") got 0 results with no error, since this
	// function always blindly concatenated base/game/relativePath even when
	// `game` was ALREADY a complete OS path, producing a bogus nested path
	// like "<fs_basepath>/C:\DOOM-3-BFG\map_pack\base/maps" that doesn't
	// exist on disk. Mirrors the existing IsOSPath(relativePath) escape
	// hatch immediately above -- same idea, one level up (the `game`
	// segment instead of the trailing `relativePath` segment). Every normal
	// caller passes `gameFolder`/"base"/"d3xp" here, none of which are ever
	// OS paths, so this is a pure addition with zero risk to the existing
	// non-custom-path behavior.
	if ( IsOSPath( game ) ) {
		idStr strGame = game;
		strGame.StripTrailing( '/' );
		strGame.StripTrailing( '\\' );
		sprintf( newPath, "%s/%s", strGame.c_str(), relativePath );
...
int idFileSystemLocal::AddResourceFile( const char * resourceFileName ) {
	...
	// PRD M4.5: this is now reachable from outside the single "current map"
	// assumption BeginLevelLoad's caller made (a map preview can mount,
	// unmount, and re-mount across several different maps in one process
	// lifetime) -- guard against mounting the same container twice, which
	// the original single-call-site version never needed to.
	const int existing = FindResourceFile( resourceFile.c_str() );
	if ( existing >= 0 ) {
		return existing;
	}
	...
	delete rc;	// Fixed real bug: Init() failure used to leak this allocation
	return -1;
}
...
/*
================
idFileSystemLocal::RegisterModSearchPath

  PRD M4.5 follow-up: same search-path-append + top-level-.resources-scan
  behavior as AddGameDirectory just above, EXCEPT it never touches
  gameFolder -- that field is read by every other path-resolution call in
  this file (OSPathToRelativePath, RelativePathToOSPath, RenameFile, etc.)
  to decide where saves/screenshots/config writes land; reassigning it as a
  side effect of "let me also look inside this other directory" would
  silently redirect ALL of those into the wrong tree for the rest of the
  process's life. This is a purely additive, parallel search path, not a
  new "active mod."
================
*/
void idFileSystemLocal::RegisterModSearchPath( const char * gameDir ) {
	if ( gameDir == NULL || gameDir[0] == '\0' ) {
		return;
	}
	// an absolute gameDir (see the IsOSPath(game) branch BuildOSPath just
	// gained above) resolves to the exact same physical directory no matter
	// which base it's paired with, so only one searchpath_t entry is
	// needed -- registering it against both fs_basepath and fs_savepath
	// would just make every file under it show up twice in scans.
	const int numBases = IsOSPath( gameDir ) ? 1 : 2;
	const char * const bases[2] = { fs_basepath.GetString(), fs_savepath.GetString() };
	for ( int b = 0; b < numBases; b++ ) {
		const char * path = bases[b];
		if ( path == NULL || path[0] == '\0' ) {
			continue;
		}
		if ( b == 1 && idStr::Icmp( bases[1], bases[0] ) == 0 ) {
			continue;	// fs_savepath == fs_basepath on some configs -- don't double-register
		}

		bool alreadyRegistered = false;
		for ( int i = 0; i < searchPaths.Num(); i++ ) {
			if ( searchPaths[i].path.Cmp( path ) == 0 && searchPaths[i].gamedir.Cmp( gameDir ) == 0 ) {
		...
		searchpath_t & search = searchPaths.Alloc();
		search.path = path;
		search.gamedir = gameDir;

		idStr pakfile = BuildOSPath( path, gameDir, "" );
		pakfile[ pakfile.Length() - 1 ] = 0;	// strip the trailing slash
		...
		for ( int i = 0; i < pakfiles.Num(); i++ ) {
			idResourceContainer * rc = new idResourceContainer();
			if ( rc->Init( pakfiles[i], resourceFiles.Num() ) ) {
				resourceFiles.Append( rc );
				common->Printf( "Loaded resource file %s\n", pakfiles[i].c_str() );
			} else {
				delete rc;
		...
		common->Printf( "RegisterModSearchPath: added %s/%s\n", path, gameDir );
	}
}
