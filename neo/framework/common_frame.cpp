...
void idCommonLocal::Draw() {
...

	{
		...
		// VIS: draw the visualizer as the scene, BEFORE the console, so the console
		// overlays it when pulled down and the feedback capture excludes console text
		extern void Vis_Draw2D();
		Vis_Draw2D();
...
void idCommonLocal::Frame() {
	try {
		...
		// VIS: the ImGui layer-editor tool window (win_imgui.cpp) is a separate
		// top-level window sharing this thread's cursor state (ShowCursor's
		// hide count is per-thread, not per-window) -- without this, the
		// cursor stays invisible over the tool window whenever gameplay mouse
		// capture is active, since SetCursor() alone can't override a negative
		// ShowCursor count. Checks "...Focused", not just "...Editor" (is it
		// merely open), so the cursor hides over the visualization display as
		// normal and only becomes visible while the mouse is actually over the
		// tool window (a WindowFromPoint-based geometric check against the
		// live cursor position, updated every frame in
		// Vis_ImGuiSetToolWindowVisible -- deliberately not focus-based, see
		// that function's own comment). Same "look up a vis_ cvar by name, no
		// header dependency" idiom already used for vis_backgroundAudio
		// elsewhere in this codebase's win32 layer.
		bool chatting = false;
		if ( console->Active() || Dialog().IsDialogActive() || session->IsSystemUIShowing() || ( game && game->InhibitControls() && !IsPlayingDoomClassic() ) || cvarSystem->GetCVarBool( "vis_imguiEditorFocused" ) ) {
			Sys_GrabMouseCursor( false );
