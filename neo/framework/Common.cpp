...
#include "../sound/visualizer_manager.h"   // VIS: picker overlay input hook
...
bool idCommonLocal::ProcessEvent( const sysEvent_t *event ) {
	...
	// VIS: the visualizer picker overlay claims input when open (and F1 toggles
	// it) - placed after the console so '~' still works, before the no-map
	// fallback below that would otherwise force every key into the console.
	if ( g_visualizerManager.MenuProcessEvent( event ) ) {
		return true;
	}
