...
public:
	...
	// PRD M4.5: mounts "maps/<resourceFileName>" as a resource container search
	// path (same mechanism BeginLevelLoad uses for the current map, exposed
	// directly here so a geometry-only map preview can mount a packed
	// .resources map without any of BeginLevelLoad's other gameplay-adjacent
	// side effects -- preload-buffer setup, background-cache toggling,
	// manifest/preload-list clearing). Returns the container's index, or -1
	// if the file couldn't be opened/parsed. Safe to call again for a
	// container that's already mounted (returns its existing index rather
	// than mounting a duplicate) -- pair with UnloadResourceContainer to
	// unmount before loading a different map.
	virtual int				AddResourceFile( const char * resourceFileName ) = 0;
	// PRD M4.5 follow-up: registers "<fs_basepath>/<gameDir>" (and, if set,
	// "<fs_savepath>/<gameDir>") as an ADDITIONAL search path -- without any
	// of AddGameDirectory/SetupGameDirectories' "this is now THE active mod"
	// side effects (reassigning the internal gameFolder, which every other
	// path-resolution call in this file reads for saves/screenshots/config
	// writes -- mutating it here would silently redirect ALL of those into
	// the wrong directory tree for the rest of the process's life, a much
	// wider blast radius than "let me look at this map pack's files").
	// Mirrors AddGameDirectory's own dedup check (a no-op if this exact
	// path+dir pair is already registered) and its "auto-mount any top-
	// level .resources pak files found in the new directory" behavior.
	// Lets a caller (e.g. the map-flythrough preview) point at a brand-new
	// mod/map-pack folder the user just dropped in without restarting the
	// engine via fs_game + relaunch -- fs_game/fs_game_base themselves stay
	// CVAR_INIT (boot-time only) and untouched; this is a purely additive,
	// parallel mechanism.
	virtual void			RegisterModSearchPath( const char * gameDir ) = 0;
