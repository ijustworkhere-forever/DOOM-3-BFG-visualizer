...
void TBuiltIns::initialize()
{
...

   {
      ...
      // Real HLSL allows a scalar second/third arg to broadcast against a
      // vector first arg (min(vec, scalar) is extremely common, e.g. in
      // real MilkDrop preset shaders) -- this vendored library only declared
      // the same-type overloads above, so a call like this had no exactly-
      // matching declared overload (only an implicit-cast/promotion path).
      // Declaring these explicitly is a legitimate, worthwhile fix on its
      // own merits (exact matches beat implicit conversions during overload
      // resolution, and this is one less path through that resolution code).
      // NOTE: this alone does NOT fix the real hang found in this shader
      // (bisected to a minimal 2-line repro, clamp(tan(vec2),-12,12)) --
      // that hang was confirmed (by hand-tracing findFunction ->
      // promoteFunctionArguments -> constructBuiltIn/ir_add_unary_math ->
      // TIntermUnary::promote, and by re-testing after this exact change)
      // to be somewhere else in this vendored parser, not isolated despite
      // real effort. See neo/sound/MilkShaderTranspiler.cpp for how the
      // actual hang is contained (timeout-guarded worker thread) rather
      // than root-caused inside this fork.
      s.append(TString("float2  min(float2  x, float y);"));
      s.append(TString("float3  min(float3  x, float y);"));
      s.append(TString("float4  min(float4  x, float y);"));
      ...
      s.append(TString("float2  max(float2  x, float y);"));
      s.append(TString("float3  max(float3  x, float y);"));
      s.append(TString("float4  max(float4  x, float y);"));
      ...
      s.append(TString("float2  clamp(float2  x, float minVal, float maxVal);"));
      s.append(TString("float3  clamp(float3  x, float minVal, float maxVal);"));
      s.append(TString("float4  clamp(float4  x, float minVal, float maxVal);"));
