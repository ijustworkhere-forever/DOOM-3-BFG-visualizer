...
//
// The original body here (`!IsNumeric(left)` unconditionally returning true, and
// the numeric case comparing with `>=` instead of `>`) is not a strict weak
// ordering: two non-numeric parameters, or two numeric parameters with equal
// dimensions (e.g. two scalar constants, as in the extremely common HLSL idiom
// `clamp(v, -1, 1)`), satisfy the comparator in BOTH directions at once. libstdc++
// -- and MSVC's checked-iterator std::sort in particular -- can read out of
// bounds, loop, or (in a debug CRT) trip an "invalid comparator" assertion when
// that happens; this is the confirmed root cause of both a permanent parser
// "hang" (contained separately by MilkShaderTranspiler's worker-thread timeout)
// and a real Debug-build crash on ordinary preset shader text. Rewritten as a
// lexicographic comparison of an explicit integer key, which is always a valid
// strict weak ordering (irreflexive, antisymmetric, transitive) while
// preserving the original intent: non-numeric parameters first, then numeric
// parameters by descending dimension.
static void ParameterSortKey(const TParameter &param, int outKey[3]) {
	const bool numeric = IsNumeric(param.type->getBasicType());
	outKey[0] = numeric ? 1 : 0;
	outKey[1] = numeric ? -param.type->getColsCount() : 0;
	outKey[2] = numeric ? -param.type->getRowsCount() : 0;
}

bool parameterSizeSortFunction(TParameter left, TParameter right) {
	int leftKey[3], rightKey[3];
	ParameterSortKey(left, leftKey);
	ParameterSortKey(right, rightKey);
	for (int i = 0; i < 3; i++) {
