...
#include "snd_local.h"
#include "visualizer_data.h"        // VIS: audio analyzer
#include "visualizer_manager.h"     // VIS: playlist/track state
...
void idSoundSystemLocal::Render() {
	...
	// VIS: advance visualizer playback state and run this frame's audio analysis.
	// Render() runs after the frame's visuals are committed, so consumers always
	// see the previous frame's analysis (one frame of latency by design).
	g_visualizerManager.Frame();
	g_audioAnalyzer.Update();
