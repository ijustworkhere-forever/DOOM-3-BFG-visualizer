...
bool idSoundShader::SetDefaultText() {
	...
	if ( 1 ) { //fileSystem->ReadFile( wavname, NULL ) != -1 ) {
		char generated[2048];
		// Quote the sample path so filenames containing spaces (e.g.
		// "music/Ocean Eyes.wav") parse as a single lexer token instead of
		// breaking on the first space with an "unknown token" warning.
	...
	while ( 1 ) {
		...
		// the wave files
			...
			leadin = true;
					...
					token.Find( ".mp3", false ) != -1 || token.Find( ".flac", false ) != -1 ||
					token.Find( ".m4a", false ) != -1 || token.Find( ".aac", false ) != -1 ||
					token.Find( ".opus", false ) != -1 || token.Find( ".wma", false ) != -1 ) {
