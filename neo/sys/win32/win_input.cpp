...
void IN_Frame() {
	...
	// if fullscreen, we always want the mouse -- EXCEPT when something
	// explicitly asked for it back via Sys_GrabMouseCursor(false) (console/
	// dialog/menu, or -- the case that actually surfaced this -- the
	// separate ImGui "Visualizer Layer Editor" tool window having the
	// cursor over it; see common_frame.cpp's mouse-grab condition and
	// vis_imguiEditorFocused). Previously mouseReleased was only honored in
	// windowed mode (movingWindow/!activeApp still are -- neither is
	// meaningful for an exclusive-fullscreen window, so that half of the
	// original guard is untouched), which meant the tool window's cursor
	// never showed under fullscreen -- the game's DEFAULT mode
	// (r_fullscreen defaults to 1) -- confirmed root cause of a repeated
	// direct user report ("i still do not have a mouse cursor to see where
	// i am going to click in visualizer layer editor") that survived two
	// earlier fix attempts aimed at the wrong layer (both correctly computed
	// mouseReleased, but this fullscreen guard threw the result away).
	if ( !win32.cdsFullscreen ) {
