...
#define CONSOLE_BUFFER_SIZE		16384
	...
	// VIS: tee all early-console output (including Sys_Error text) to a file next
	// to the exe, so silent startup failures are diagnosable without a desktop.
	static FILE * conLogFile = NULL;
	static bool conLogTried = false;
	if ( !conLogTried ) {
		conLogTried = true;
		char logPath[MAX_PATH];
		if ( GetModuleFileName( NULL, logPath, sizeof( logPath ) ) ) {
			char * lastSlash = strrchr( logPath, '\\' );
			if ( lastSlash != NULL && ( lastSlash - logPath ) + 20 < (int)sizeof( logPath ) ) {
				strcpy( lastSlash + 1, "early_console.log" );
				conLogFile = fopen( logPath, "w" );
			}
		}
	}
	if ( conLogFile != NULL ) {
		fputs( pMsg, conLogFile );
		fflush( conLogFile );
	}
