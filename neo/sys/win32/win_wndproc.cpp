...
#include "win_wallpaper.h"	// PRD M7: WM_DESTROY safety net, see the case below
...
LONG WINAPI MainWndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam ) {
	...
	// PRD (post-M4 follow-up): the layer-editor UI now lives in its own
	// separate top-level window (see win_imgui.cpp's design note), not an
	// overlay on this one -- ImGui no longer needs to see this window's
	// messages at all; its own tool-window WNDPROC (ImGuiToolWndProc in
	// win_imgui.cpp) calls Vis_ImGuiWndProcHandler for ITS OWN messages
	// instead. Forwarding this window's messages to ImGui too would be
	// actively wrong now (mouse/keyboard activity in the 3D game view would
	// incorrectly feed ImGui's input state for a window it's no longer
	// bound to).
	...
	switch( uMsg ) {
		...
		case WM_DESTROY:
			// Fixed real bug (found via code review, PRD M7): wallpaper mode
			// reparents this window to become a CHILD of a WorkerW HWND owned
			// by explorer.exe -- per documented Win32 behavior, DestroyWindow
			// on a parent cascades to destroy its children. If explorer.exe
			// crashes or is restarted (Task Manager "Restart Windows
			// Explorer," or a real crash) while wallpaper mode is active,
			// this WM_DESTROY fires WITHOUT going through the normal
			// WM_CLOSE -> "quit" command path just below, and the engine
			// would otherwise keep running with win32.hWnd == NULL forever --
			// no crash, just silently broken with no window and no way to
			// interact with it. Detect that specific case and force a clean
			// shutdown instead of drifting into that state.
			if ( Vis_WallpaperIsEnabled() && !common->IsShuttingDown() ) {
				idLib::Warning( "WM_DESTROY while wallpaper mode was still active (window likely destroyed out "
								 "from under the engine, e.g. by an explorer.exe crash/restart) -- shutting down" );
		...
		case WM_ACTIVATE:
			...
			// don't activate until the mouse button is released
			{
				...
				// VIS: keep audio (and thus loopback-driven reactivity) running when
				// the window loses focus, so the visualizer works in the background.
				if ( win32.activeApp || !cvarSystem->GetCVarBool( "vis_backgroundAudio" ) ) {
