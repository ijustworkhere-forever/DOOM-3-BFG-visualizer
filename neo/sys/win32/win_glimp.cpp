...
#include "win_imgui.h"	// PRD M4: init/shutdown once a window + GL context exist
...
bool GLimp_Init( glimpParms_t parms ) {
	...
	// PRD M4: this is the first point a valid HWND AND a current GL context
	// both exist (GLW_CreateWindow -> GLW_InitDriver already succeeded above),
	// which is what ImGui_ImplWin32_Init/ImGui_ImplOpenGL3_Init both need.
	// Best-effort: failure just means the layer-editor UI is unavailable,
	// not fatal to the engine.
	Vis_ImGuiInit();
...
void GLimp_Shutdown() {
	...
	// PRD M4: tear down ImGui BEFORE the GL context/window go away -- the
	// last point both are still valid (mirrors Vis_ImGuiInit's ordering
	// requirement in reverse). Safe even if Init was never called/failed.
	Vis_ImGuiShutdown();
