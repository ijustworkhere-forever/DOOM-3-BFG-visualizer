...
int Sys_ListFiles( const char *directory, const char *extension, idStrList &list ) {
	...
	// VIS: _findfirst returns intptr_t on x64; truncating to int corrupts the
	// handle passed to _findnext (intermittent infinite loops in file listing)
	intptr_t	findhandle;
	int			flag;
...
sysDLLHandle_t Sys_DLL_Load( const char *dllName ) {
	...
	return (sysDLLHandle_t)libHandle;
}
...
void HackChkStk() {
	...
	*(byte *)_chkstk = 0xe9;
	*(int *)((intptr_t)_chkstk + 1) = (int)((intptr_t)clrstk - (intptr_t)_chkstk - 5);
...
EXCEPTION_DISPOSITION __cdecl _except_handler( struct _EXCEPTION_RECORD *ExceptionRecord, void *EstablisherFrame,
	...
	sprintf( msg,
		...
		(unsigned long long)ContextRecord->Rip );
#else
	char FPUFlags[2048];
	Sys_FPU_PrintStateFlags( FPUFlags, ContextRecord->FloatSave.ControlWord,
		ContextRecord->FloatSave.StatusWord, ContextRecord->FloatSave.TagWord,
		ContextRecord->FloatSave.ErrorOffset, ContextRecord->FloatSave.ErrorSelector,
		ContextRecord->FloatSave.DataOffset, ContextRecord->FloatSave.DataSelector );
	sprintf( msg,
		"Please describe what you were doing when DOOM 3 crashed!\n"
		"-= FATAL EXCEPTION =-\n\n"
		"%s\nException code: 0x%08x\nException address: %p\n"
		...
		com_version.GetString(), (unsigned int)ExceptionRecord->ExceptionCode,
		ExceptionRecord->ExceptionAddress, ContextRecord->Eax, ContextRecord->Ebx,
		ContextRecord->Ecx, ContextRecord->Edx, ContextRecord->Eip,
		ContextRecord->Esp, ContextRecord->Ebp, FPUFlags );
#endif
...
// Legacy x86 exception-registration assembly was intentionally disabled.
	// Structured Exception Handling is installed by the Windows runtime.
