...
int Sys_GetVideoRam() {
#if defined(_WIN64)
	// The legacy WMI helper used ATL smart pointers, which are not available in
	// the x64 build environment. Keep the conservative fallback on Win64.
	return 64;
#else
...
address_t GetCallerAddr( long _ebp ) {
	// The old implementation walked an x86 EBP chain with inline assembly.
	// Frame pointers are not guaranteed on modern MSVC builds, and the code
	// cannot be compiled for x64.  Callers should use CaptureStackBackTrace
	// through Sys_GetCallStack instead.
	(void)_ebp;
	return 0;
}
...
void Sys_GetCallStack( address_t *callStack, const int callStackSize ) {
	...
	memset( callStack, 0, callStackSize * sizeof( address_t ) );

	// Frame-pointer independent stack capture for both Win32 and Win64.
	const USHORT captured = RtlCaptureStackBackTrace( 2, (ULONG)callStackSize,
		reinterpret_cast<PVOID *>( callStack ), NULL );
	(void)captured;
}
