...
// Default changed from "1" (fullscreen) to "0" (windowed) -- direct user
...
// going straight into fullscreen might confuse new users." Only affects a
// fresh install/config -- CVAR_ARCHIVE means an existing saved
// DoomConfig.cfg with r_fullscreen already written keeps whatever value
// it has, same as any other default change.
...
// PRD M4.5 map-flythrough follow-up: draw each surface's own SL_DIFFUSE stage
// through the existing non-light ambient pass (RB_DrawShaderPasses), so a
// geometry-only render world with ZERO light sources still shows real
// per-surface diffuse textures instead of solid black -- WITHOUT going through
// the interaction/shadow/AddLightDef path that twice crashed in R_AddSingleLight
// (see neo/sound/visualizer_manager.cpp's vis_mapLoad comment). Off by default:
// every use site is guarded by this cvar, so normal lit rendering is unchanged.
idCVar r_forceAmbientDiffuse( "r_forceAmbientDiffuse", "0", CVAR_RENDERER | CVAR_BOOL, "draw each surface's diffusemap unlit via the ambient pass (fullbright, no dynamic light) -- used by the map-flythrough preview for light-less geometry-only worlds" );
// PRD follow-up (direct user report: "for the map can we choose the
// wireframe hue... those should also be bindable parameters"). Tints
// RB_ShowTris's debug wireframe color when >= 0 (a full-saturation HSV
// color at the given hue); negative (the default) means "use plain white,
// exactly as before" -- every OTHER existing use of r_showTris throughout
// the engine is completely unaffected unless something explicitly sets
// this cvar, which only the map-flythrough feature does.
idCVar r_showTrisHue( "r_showTrisHue", "-1", CVAR_RENDERER | CVAR_FLOAT, "tint RB_ShowTris's debug wireframe with a full-saturation HSV color at this hue (0..1); negative (default) means plain white, unchanged from before -- used by the map-flythrough preview's bindable wireframe hue", -1.0f, 1.0f );
...
// 0-7 matches the visualizer's own VISBLEND_NORMAL..INVERT order; negative
// (default) means "use r_showTris's own built-in blend state, unchanged
// from before" -- every OTHER r_showTris user is unaffected.
idCVar r_showTrisBlendMode( "r_showTrisBlendMode", "-1", CVAR_RENDERER | CVAR_INTEGER, "override RB_ShowTris's wireframe blend mode: 0 Normal, 1 Additive, 2 Subtractive, 3 Multiply, 4 Screen, 5 Darken, 6 Lighten, 7 Invert; negative (default) leaves r_showTris's own built-in blend state unchanged -- used by the map-flythrough preview's bindable wireframe blend mode", -1, 7 );
...
// PRD Pillar C follow-up: real ZGE-style subtractive/darken/lighten layer
// blend modes need glBlendEquation -- not directly linkable on Windows (the
// system opengl32.lib only exports GL 1.1) even though it's core GL 1.4, so
// it's loaded the same GLimp_ExtensionPointer way as the ARB/EXT entries
// below rather than the qglBlendFunc-style direct-link path in win_qgl.cpp.
...
// GL_ARB_framebuffer_object (VIS: PRD M1 step 8)
...
static void R_CheckPortableExtensions() {
	...
	// PRD Pillar C follow-up: glBlendEquation is core GL 1.4 (not an ARB/EXT
	// extension needing its own GL_ARB_* capability string check), but still
	// needs GLimp_ExtensionPointer on Windows since opengl32.lib only
	// exports GL 1.1 directly -- load unconditionally, no availability gate.
	...
	// GL_ARB_framebuffer_object (VIS: PRD M1 step 8 -- off-screen half-float
	// render target, the actual fix for MilkDrop-style feedback banding;
	// core since GL 3.0 so this should always be available on this build's
	// target hardware, but gated the same defensive way every other
	// extension here is)
	...
	// Fixed real bug: qglVertexAttribPointerARB/qglEnableVertexAttribArrayARB/
	// qglDisableVertexAttribArrayARB (used for EVERY vertex draw call, see
	// tr_backend_draw.cpp) used to only get loaded inside this
	// GL_ARB_fragment_program block, even though they've been CORE OpenGL
	// (not extension-gated) since 2.0 -- vertex_attrib_pointer / enable/
	// disable_vertex_attrib_array were promoted to core well before this
	// engine's minimum GL 2.0 requirement, entirely independent of the old
	// ARB assembly-shader extension. This engine only ever loads/uses GLSL
	// shaders (see idRenderProgManager::LoadVertexShader/LoadFragmentShader
	// -> LoadGLSLShader in RenderProgs.cpp); the ARB assembly-program path
	// below (LoadShader, qglProgramStringARB et al) is dead code, never
	// called from anywhere. Confirmed live: under RenderDoc's OpenGL
	// capture (which requires/forces a 3.2 CORE profile context, and
	// GL_ARB_fragment_program -- a compatibility-profile-only legacy
	// extension -- isn't exposed there), this coupling meant the vertex
	// attrib functions never loaded, and the engine's own hard requirement
	// check below then fatally errored out during init, permanently
	// killing the OpenGL context and leaving the process stuck pumping an
	// empty message loop forever. Loading the vertex-attrib functions
	// unconditionally (via their core, non-ARB-suffixed names, which
	// exist across compatibility AND core profiles alike) fixes this
	// without touching any actually-used rendering behavior.
	...
	qglGetIntegerv( GL_MAX_TEXTURE_COORDS, (GLint *)&glConfig.maxTextureCoords );
	qglGetIntegerv( GL_MAX_TEXTURE_IMAGE_UNITS, (GLint *)&glConfig.maxTextureImageUnits );

	// GL_ARB_vertex_program / GL_ARB_fragment_program: only the legacy ARB
	// assembly-shader path (dead code, see comment above) actually depends
	...
	// Fixed real bug: this used to be a fatal idLib::Error, but nothing this
	// engine actually renders depends on GL_ARB_fragment_program -- see the
	// long comment where fragmentProgramAvailable is set, above. The
	// vertex-attrib functions this used to gate now load unconditionally
	// (via their core names), and the ARB assembly-shader path this
	// extension WOULD gate is dead code, never called. Treating its
	// absence as fatal only ever hurt: it made this engine impossible to
	// run under RenderDoc's OpenGL capture (which requires/forces a 3.2
	// core profile, and this is a compatibility-profile-only legacy
	// extension there) -- confirmed live, the fatal error tore down the
	// whole OpenGL context and left the process stuck in an empty message
	// loop forever, with no way to recover short of killing the process.
	if ( !glConfig.fragmentProgramAvailable ) {
		idLib::Warning( "GL_ARB_fragment_program not available (harmless -- only the unused legacy ARB assembly-shader path needs it)" );
	}
	// GLSL
	if ( !glConfig.glslAvailable ) {
...
void R_InitMaterials() {
	...
	// DATALESS-BOOT FIX: the intrinsic _white material has no `blend` stage, so
	// DrawFilled() (console background, HUD fills, etc.) renders nothing in this
	// build. Prefer the boot material visualizer/solid (a real white TGA + blend +
	// vertexColor) which paints correctly; fall back to _white when it's absent
	// (a normal data-present build).
	tr.whiteMaterial = declManager->FindMaterial( "visualizer/solid", false );
	if ( !tr.whiteMaterial ) {
