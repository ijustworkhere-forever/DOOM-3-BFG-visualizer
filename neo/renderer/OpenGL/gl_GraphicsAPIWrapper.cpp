...
void GL_SetDefaultState() {
	...
	// Fixed real bug: glShadeModel is a fixed-function-pipeline call (flat
	// vs. smooth shading) with zero effect on this engine's GLSL-only
	// rendering -- every draw's actual shading is fully determined by its
	// fragment shader, which doesn't read or care about this legacy state
	// at all. It's also not a valid call under a core OpenGL profile
	// (removed entirely, fixed-function-only). Confirmed via RenderDoc's
	// OpenGL capture support (which requires a core profile context, see
	// r_useOpenGL32's comment) explicitly flagging this exact call as
	// "Unsupported function used" and disabling captures because of it --
	// removing this vestigial, do-nothing call is what let RenderDoc
	// actually capture a frame from this engine for the first time.
...
void GL_State( uint64 stateBits, bool forceGlState ) {
	...
	//
	// check blend equation (PRD Pillar C follow-up: real subtractive/darken/
	// lighten layer blend modes, not just additive -- this field previously
	// existed in GLState.h but was never read here at all, so it was a
	// complete no-op everywhere it was already set, e.g. SWF_Render.cpp's
	// Flash blend modes). Always resolve explicitly (not just on a 0->nonzero
	// diff) since glBlendEquation is sticky GL state that persists across
	// unrelated draws once anything ever calls it -- every state change must
	// explicitly reassert ADD when no special mode bits are set, or a
	// previous draw's subtractive/min/max mode would silently leak forward.
	//
	if ( diff & GLS_BLENDOP_BITS ) {
		GLenum blendEquation = GL_FUNC_ADD;
		switch ( stateBits & GLS_BLENDOP_BITS ) {
			case GLS_BLENDOP_ADD:		blendEquation = GL_FUNC_ADD; break;
			case GLS_BLENDOP_SUB:		blendEquation = GL_FUNC_SUBTRACT; break;
			case GLS_BLENDOP_REVSUB:	blendEquation = GL_FUNC_REVERSE_SUBTRACT; break;
			case GLS_BLENDOP_MIN:		blendEquation = GL_MIN; break;
			case GLS_BLENDOP_MAX:		blendEquation = GL_MAX; break;
			default:
				assert( !"GL_State: invalid blend op state bits\n" );
				break;
		}
		qglBlendEquation( blendEquation );
	}
