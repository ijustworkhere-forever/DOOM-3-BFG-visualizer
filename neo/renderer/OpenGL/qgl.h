...
// system opengl32.lib only exports GL 1.1) even though it's core GL 1.4 --
// loaded via GLimp_ExtensionPointer like the other entries in this section,
// not the qglBlendFunc-style direct-link path above.
...
// feedback render target -- see plans/PRD-implementation-status.md)
...
// PRD M3: MilkDrop custom warp shaders bind uniforms by name (sampler_main,
// bass, q1, _qa...) rather than through the engine's own RENDERPARM_/
// rpUser<N> convention (see RB_VisMilkWarpDraw), so those raw scalar/vec2/
// mat2 setters are needed here too, not just the vec4-only qglUniform4fv
// the engine's own shaders have always been enough for.
