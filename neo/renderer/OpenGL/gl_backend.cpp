	...
	for ( int stereoEye = 1; stereoEye >= -1; stereoEye -= 2 ) {
		...
		for ( const emptyCommand_t * cmds = allCmds; cmds != NULL; cmds = (const emptyCommand_t *)cmds->next ) {
			...
			case RC_POST_PROCESS:
				...
				break;
			case RC_VIS_FBO_BEGIN:
			case RC_VIS_FBO_END:
				// PRD M1 step 8: the visualizer's offscreen redirect isn't
				// stereo-aware (it renders one flat 2D overlay, not a
				// per-eye view) -- deliberately a no-op here rather than
				// falling into the "bad commandId" error below, since a
				// mismatched Begin/End would otherwise leave draws captured
				// in a dangling per-eye redirect. Front-end callers should
				// treat stereo mode as out of scope for now.
				break;
			case RC_IMGUI_RENDER:
				// PRD M4: this inner loop walks the WHOLE command list once
				// per eye (see RC_POST_PROCESS's own viewEyeBuffer guard
				// just above, which exists for the same reason) -- but
				// RB_ImGuiRender frees its heap-allocated draw-data clone
				// after submitting it, so calling it on both passes would
				// be a use-after-free/double-free, not just a harmless
				// double-render. Only process on the first eye pass; a flat
				// UI overlay rendering once (not per-eye) is an acceptable
				// simplification for a debug/editor panel, not a gameplay-
				// critical stereo feature.
				if ( stereoEye == 1 ) {
					RB_ImGuiRender( cmds );
				}
				break;
			case RC_CAPTURE_RENDER:
				// PRD M6/M7: same per-eye double-processing concern as
				// RC_IMGUI_RENDER just above -- a capture/NDI-send isn't
				// stereo-aware either, so only do it once per frame.
				if ( stereoEye == 1 ) {
					RB_VisCapture( cmds );
				}
				break;
			case RC_VIS_MILK_SHADER_COMPILE:
			case RC_VIS_MILK_WARP_DRAW:
			case RC_VIS_MILK_MASH_DRAW:
				// PRD M3/M5: same reasoning as RC_VIS_FBO_BEGIN/END just above --
				// the MilkDrop custom-shader warp path (and its M5 mash-up B
				// side) renders one flat 2D overlay, not a per-eye view;
				// deliberately a no-op here rather than the "bad commandId"
				// error below. Stereo mode is out of scope for the whole
				// visualizer feature already.
				break;
			default:
...
void RB_ExecuteBackEndCommands( const emptyCommand_t *cmds ) {
	...
	for ( ; cmds != NULL; cmds = (const emptyCommand_t *)cmds->next ) {
		...
		case RC_CAPTURE_RENDER:
			...
			break;
		case RC_VIS_MILK_SHADER_COMPILE:
			RB_VisMilkShaderCompile( cmds );
			break;
		case RC_VIS_MILK_WARP_DRAW:
			RB_VisMilkWarpDraw( cmds );
			break;
		case RC_VIS_MILK_MASH_DRAW:
			RB_VisMilkMashDraw( cmds );
			break;
		default:
			common->Error( "RB_ExecuteBackEndCommands: bad commandId" );
			break;
