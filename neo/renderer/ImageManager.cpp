...
idImage *idImageManager::ImageFromFunction( const char *_name, void (*generatorFunction)( idImage *image ) ) {
	...
	for ( int i = imageHash.First( hash ); i != -1; i = imageHash.Next( i ) ) {
		...
		if ( name.Icmp( image->GetName() ) == 0 ) {
			// Fixed real bug: an image can already be registered under this
			// name with NO generatorFunction (e.g. a material's Bind()
			// on-demand load created a bare placeholder for it before this
			// call ever ran -- confirmed via live testing: a caller trying
			// to attach a generator here to silence a "Couldn't load image"
			// warning saw the warning keep flooding, because this branch
			// used to just return the existing, still-generator-less image
			// untouched. Attach it now instead of leaving it permanently
			// stuck attempting (and failing) a real file load every time
			// something calls Bind() on it.
