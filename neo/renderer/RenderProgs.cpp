...
void idRenderProgManager::SetRenderParms( renderParm_t rp, const float * value, int num ) {
	for ( int i = 0; i < num; i += 4 ) {
		SetRenderParm( (renderParm_t)(rp + i/4), value + i );
	}
}
...
int idRenderProgManager::LoadShaderFromSource( const char * vertexSource, const char * fragmentSource, idList<int> & uniforms ) {
	(void)uniforms;
	if ( vertexSource == NULL || fragmentSource == NULL || qglCreateShader == NULL || qglCreateProgram == NULL ) {
		return -1;
	}

	const GLuint vertShader = qglCreateShader( GL_VERTEX_SHADER );
	qglShaderSource( vertShader, 1, &vertexSource, NULL );
	qglCompileShader( vertShader );
	GLint status = GL_FALSE;
	qglGetShaderiv( vertShader, GL_COMPILE_STATUS, &status );
	if ( status == GL_FALSE ) {
		qglDeleteShader( vertShader );
		return -1;
	}

	const GLuint fragShader = qglCreateShader( GL_FRAGMENT_SHADER );
	qglShaderSource( fragShader, 1, &fragmentSource, NULL );
	qglCompileShader( fragShader );
	qglGetShaderiv( fragShader, GL_COMPILE_STATUS, &status );
	if ( status == GL_FALSE ) {
		qglDeleteShader( vertShader );
	...
	const GLuint progId = qglCreateProgram();
	qglAttachShader( progId, vertShader );
	qglAttachShader( progId, fragShader );
	qglLinkProgram( progId );
	qglGetProgramiv( progId, GL_LINK_STATUS, &status );
	qglDeleteShader( vertShader );
	qglDeleteShader( fragShader );
	if ( status == GL_FALSE ) {
		qglDeleteProgram( progId );
		return -1;
	}
	return ( int )progId;
}
