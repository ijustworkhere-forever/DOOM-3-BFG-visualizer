...
void idImage::CopyFramebuffer( int x, int y, int imageWidth, int imageHeight ) {
	...
	// VIS: honor a pre-set FMT_RGBA16F (see idImage::SetFormat) instead of
	// always hardcoding 8-bit -- callers that need a higher-precision render
	// target (e.g. future FBO-based feedback accumulation) can opt in.
	const GLenum copyInternalFormat = ( opts.format == FMT_RGBA16F ) ? GL_RGBA16F : GL_RGBA8;
	qglCopyTexImage2D( GL_TEXTURE_2D, 0, copyInternalFormat, x, y, imageWidth, imageHeight, 0 );
