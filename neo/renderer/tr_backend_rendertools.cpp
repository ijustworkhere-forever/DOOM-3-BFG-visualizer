...
static void RB_ShowTris( drawSurf_t **drawSurfs, int numDrawSurfs ) {
	...
	// wireframe hue ... those should also be bindable parameters"). Plain
	// inline HSV->RGB (full saturation/value) rather than pulling in the
	// visualizer's own HueColor() helper -- this is core renderer code with
	// no dependency on neo/sound/, and every OTHER r_showTris user is
	// unaffected since r_showTrisHue defaults to -1 (disabled).
	if ( r_showTrisHue.GetFloat() >= 0.0f ) {
		const float h = ( r_showTrisHue.GetFloat() - idMath::Floor( r_showTrisHue.GetFloat() ) ) * 6.0f;
		const int   hi = (int)h;
		const float f = h - (float)hi;
		...
		switch ( hi ) {
			...
			default: color[0] = 1; color[1] = 0; color[2] = q; break;
		}
	}
	...
	switch ( r_showTris.GetInteger() ) {
		case 1: // only draw visible ones
			stateBits = GLS_DEPTHMASK | GLS_ALPHAMASK | GLS_POLYMODE_LINE | GLS_POLYGON_OFFSET;
			break;
		case 2:	// draw all front facing
		case 3: // draw all
			stateBits = GLS_DEPTHMASK | GLS_ALPHAMASK | GLS_POLYMODE_LINE | GLS_POLYGON_OFFSET | GLS_DEPTHFUNC_ALWAYS;
			break;
		case 4: // only draw visible ones with blended lines
			stateBits = GLS_DEPTHMASK | GLS_ALPHAMASK | GLS_POLYMODE_LINE | GLS_POLYGON_OFFSET | GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA;
			color[3] = 0.4f;
			break;
	}

	// PRD follow-up (direct user report: "we need blend modes for the map
	// flythrough layer ... wireframe could lighten"). Overrides just the
	// blend factor/equation bits with the same combos Material.cpp's "blend"
	// keyword uses (blend/add/subtract/filter/screen/darken/lighten/invert),
	// leaving depth/polymode/offset from the r_showTris mode above untouched.
	// r_showTrisBlendMode defaults to -1 (disabled), so every OTHER
	// r_showTris user is unaffected unless something explicitly sets it.
	if ( r_showTrisBlendMode.GetInteger() >= 0 ) {
		stateBits &= ~( GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS | GLS_BLENDOP_BITS );
		switch ( r_showTrisBlendMode.GetInteger() ) {
			case 1: stateBits |= GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE; break;                                          // Additive
			case 2: stateBits |= GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_BLENDOP_REVSUB; break;                     // Subtractive
			case 3: stateBits |= GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO; break;                                   // Multiply
			case 4: stateBits |= GLS_SRCBLEND_ONE_MINUS_DST_COLOR | GLS_DSTBLEND_ONE; break;                          // Screen
			case 5: stateBits |= GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_BLENDOP_MIN; break;                        // Darken
			case 6: stateBits |= GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_BLENDOP_MAX; break;                        // Lighten
			case 7: stateBits |= GLS_SRCBLEND_ONE_MINUS_DST_COLOR | GLS_DSTBLEND_ZERO; break;                         // Invert
			default: stateBits |= GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA; break;                   // Normal
		}
	}
	GL_State( stateBits );
