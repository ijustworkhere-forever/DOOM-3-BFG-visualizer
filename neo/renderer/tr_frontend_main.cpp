...
12
In addition, the Doom 3 BFG Edition Source Code is also subject to certain additional terms. You should have received a copy of these additional terms immediately following the terms and conditions of the GNU General Public License which accompanied the Doom 3 BFG Edition Source Code.  If not, please request a copy in writing to id Software LLC, c/o ZeniMax Media Inc., Suite 120, Rockville, Maryland 20850 USA.
22
===========================================================================
*/
...
void R_ToggleSmpFrame() {
	...
	const unsigned int bytesNeededForAlignment = FRAME_ALLOC_ALIGNMENT - ( ( unsigned int )( reinterpret_cast<uintptr_t>( frameData->frameMemory ) ) & ( FRAME_ALLOC_ALIGNMENT - 1 ) );
...
static void R_SortDrawSurfs( drawSurf_t ** drawSurfs, const int numDrawSurfs ) {
		...
		for ( ; ; ) {
			...
			if ( j - i >= 4 && level < MAX_LEVELS - 1 ) {
				...
				//while ( indices[j] == pivot && st_lo < i++ ) i++;
		...
		// draw in the order they were added, within a given model.
...
void R_RenderView( viewDef_t *parms ) {
	...
	idRenderMatrix::Multiply( *(idRenderMatrix *)tr.viewDef->projectionMatrix, viewRenderMatrix, tr.viewDef->worldSpace.mvp );
	...
	// write everything needed for the demo file
	if ( common->WriteDemo() ) {
...
viewDef_t *parms: the view being rendered
================
*/
