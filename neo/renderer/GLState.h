...
// PRD Pillar C follow-up (real ZGE-style layer blend modes: additive/
// subtractive/darken/lighten, not just additive/normal). Moved from bits
// 18-19 (2 bits: ADD/SUB/MIN/MAX only) into the free 15-17 gap and widened
// to 3 bits so REVERSE_SUB fits too (5 values need 3 bits, not 2). This
// field was previously NEVER read by GL_State at all -- confirmed dead:
// SWF_Render.cpp already builds GLS_BLENDOP_SUB/MIN/MAX for its own Flash
// blend modes expecting them to take effect, but since nothing consumed
// this field, those blend modes have silently no-op'd since this code was
// written. Wiring GL_State to actually honor it (gl_GraphicsAPIWrapper.cpp)
// fixes that pre-existing bug as a side effect, not just adding new
// visualizer capability -- moving the bit position is safe precisely
// because it was never observably read anywhere.
static const uint64 GLS_BLENDOP_ADD					= 0ull << 15;
static const uint64 GLS_BLENDOP_SUB						= 1ull << 15;
static const uint64 GLS_BLENDOP_REVSUB					= 2ull << 15;
static const uint64 GLS_BLENDOP_MIN						= 3ull << 15;
static const uint64 GLS_BLENDOP_MAX						= 4ull << 15;
static const uint64 GLS_BLENDOP_BITS					= 7ull << 15;
