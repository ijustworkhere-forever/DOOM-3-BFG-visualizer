...
void idMaterial::ParseBlend( idLexer &src, shaderStage_t *stage ) {
	...
	// additive but need all the modes, additive, subtractive, inverse and
	// the rest"). GLS_BLENDOP_* was previously defined in GLState.h but
	// never read by GL_State at all, so "subtract"/"darken"/"lighten" below
	// genuinely couldn't have worked before that was fixed
	// (gl_GraphicsAPIWrapper.cpp) alongside this. "screen"/"invert" need no
	// new equation support -- they're expressible with the existing
	// always-ADD factor-pair scheme, same as "blend"/"add"/"filter" above.
	if ( !token.Icmp( "subtract" ) ) {
		stage->drawStateBits = GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_BLENDOP_REVSUB;
		return;
	}
	if ( !token.Icmp( "screen" ) ) {
		stage->drawStateBits = GLS_SRCBLEND_ONE_MINUS_DST_COLOR | GLS_DSTBLEND_ONE;
		return;
	}
	if ( !token.Icmp( "darken" ) ) {
		stage->drawStateBits = GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_BLENDOP_MIN;
		return;
	}
	if ( !token.Icmp( "lighten" ) ) {
		stage->drawStateBits = GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_BLENDOP_MAX;
		return;
	}
	if ( !token.Icmp( "invert" ) ) {
		stage->drawStateBits = GLS_SRCBLEND_ONE_MINUS_DST_COLOR | GLS_DSTBLEND_ZERO;
		return;
	}
