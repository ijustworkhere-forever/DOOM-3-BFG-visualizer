...
enum renderCommand_t {
	...
	RC_COPY_RENDER,
	RC_POST_PROCESS,
	RC_VIS_FBO_BEGIN,	// PRD M1 step 8: redirect subsequent 2D draws into an offscreen FBO
	RC_VIS_FBO_END,		// ...then blit that FBO's color texture back to the backbuffer
	RC_IMGUI_RENDER,	// PRD M4: submit a deep-copied ImGui draw-data snapshot to the GPU
	RC_CAPTURE_RENDER,	// PRD M6/M7: qglReadPixels on the thread that owns the GL context, not Draw2D's
	RC_VIS_MILK_SHADER_COMPILE,	// PRD M3: compile a preset's transpiled GLSL warp/comp shader
	RC_VIS_MILK_WARP_DRAW,		// PRD M3: draw a full-screen quad with that shader, if compiled
	RC_VIS_MILK_MASH_DRAW,		// PRD M5: draw the mash-up B side's own warp/comp shader, alpha-blended on top
};
...
// PRD M1 step 8: unlike copyRenderCommand_t (which copies an ALREADY-drawn
// backbuffer -- capped at the window's fixed-point pixel format, see
// idImage::CopyFramebuffer), these bracket a run of drawSurfsCommand_t's so
// they render directly INTO colorImage's own texture (e.g. FMT_RGBA16F),
// avoiding that precision cap.
//
// Fixed real bug: colorImage used to be (re)allocated to the target size by
// idRenderSystemLocal::BeginOffscreenRender itself, on the FRONT END --
// but BeginOffscreenRender is called from idVisualizerManager::Draw2D,
// which runs on the game/draw worker thread, not the thread that owns the
// GL context. idImage::AllocImage does raw qglGenTextures/qglTexImage2D
// calls immediately, synchronously, on whatever thread calls it -- doing
// that from the wrong thread produced a texture object the backend's own
// GL context didn't consistently see as valid, causing
// "RB_VisFboBegin: incomplete framebuffer" (GL_FRAMEBUFFER_INCOMPLETE_
// MISSING_ATTACHMENT) and a black visualizer. This is the same class of
// bug already fixed once in this file for RB_VisCapture (see its comment
// in RenderSystem.cpp) -- raw GL work belongs on the backend. Now the
// front end just requests the target size/format here; RB_VisFboBegin
// does the actual (re)allocation itself, on the correct thread, before
// attaching the texture to the framebuffer.
struct visFboBeginCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	idImage *			colorImage;
	idImageOpts			targetOpts;
};

struct visFboEndCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	idImage *			colorImage;		// same image as the matching Begin
};

// PRD M4: carries a deep-copied ImGui draw-data snapshot (built/Render()'d on
// the main thread, since ImGui's own context isn't safe to touch from two
// threads at once) to whichever thread actually owns the GL context. Kept as
// an opaque `void *` (not `ImDrawData *`) so this renderer-core header never
// needs to know ImGui's types exist -- RB_ImGuiRender (implemented in
// neo/sys/win32/win_imgui.cpp, the one file that already includes ImGui)
// casts it back and is responsible for freeing the clone after submitting it.
struct imguiRenderCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	void *				drawDataClone;
};

// PRD M3: carries a preset's transpiled warp/comp GLSL (idMilkShaderTranspiler)
// to the thread that owns the GL context for compiling+linking -- raw
// qglCreateShader/qglCompileShader/qglLinkProgram calls (via
// idRenderProgManager::LoadShaderFromSource) must happen there, same rule as
// every other raw-GL command in this file. vertexSource/fragmentSource are
// heap-allocated by the front end (EnqueueVisMilkShaderCompile) and freed by
// RB_VisMilkShaderCompile after use.
struct visMilkShaderCompileCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	idStr *				vertexSource;
	idStr *				fragmentSource;
	// PRD M3: MilkDrop's separate post-composite shader, run as a SECOND
	// full-screen pass on the already-warped image (see RB_VisMilkCompDraw's
	// comment in tr_backend_draw.cpp) -- NULL if the preset has no comp_
	// shader or it failed to transpile, in which case only the warp pass
	// runs, same single-pass behavior as before comp_ support existed.
	idStr *				compFragmentSource;
	// PRD M5: true when this compile is for the mash-up "B" side (a second,
	// independently-compiled warp_/comp_ program pair kept alongside the "A"
	// side's, so both can be drawn -- and alpha-blended together -- the same
	// frame) rather than the active preset's own "A" side.
	bool				isMashB;
};

// PRD M3: draws one full-screen quad with the most recently compiled
// MilkDrop custom warp/comp shader (see RB_VisMilkShaderCompile), passing
// the preset's own uniform preamble (MilkShaderTranspiler.cpp's
// kMilkPreamble) as raw GLSL uniforms -- this bypasses the engine's
// RENDERPARM_/rpUser<N> uniform system entirely since those names don't
// match MilkDrop's naming convention (sampler_main, bass, q1, ...).
struct visMilkWarpDrawCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	idImage *			mainImage;		// bound as sampler_main
	// PRD M3: intermediate target for the warp_->comp_ two-pass pipeline --
	// only actually used (bound as an offscreen render target for the warp
	// pass, then as comp_'s own sampler_main) when a comp_ shader compiled
	// successfully; harmless to always resolve/pass otherwise.
	idImage *			intermediateImage;
	visMilkWarpUniforms_t	uniforms;
};

// PRD M5: draws the mash-up "B" side's own warp_/comp_ program pair (see
// RB_VisMilkMashDraw in tr_backend_draw.cpp) alpha-blended on top of
// whatever's already in the destination framebuffer -- issued right after
// RC_VIS_MILK_WARP_DRAW within the same frame's Draw2D pass, same mainImage
// (both A and B warp the SAME captured feedback frame, only the shader
// differs -- matches the CPU-mesh mash-up's "same material, different
// sampled UVs" precedent).
struct visMilkMashDrawCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	idImage *			mainImage;
	idImage *			intermediateImage;	// B's own warp_->comp_ intermediate (reuses the same image as A's, safe: A's usage is fully consumed by the time B draws)
	visMilkWarpUniforms_t	uniforms;
	float				mashMix;			// 0..1, B's compositing alpha -- NOT applied to A, which stays opaque
};

// PRD M6/M7: defers qglReadPixels to backend command execution, same
// reasoning as imguiRenderCommand_t above. Screen recording and NDI
// output both used to call CaptureRenderToFile/CaptureRenderToMemory
// directly from Draw2D, which runs on the SMP draw-worker thread, not
// the thread that owns the GL context -- a confirmed real crash
// (non-deterministic fault offset each run, the classic cross-thread GL
// symptom). RB_VisCapture (RenderSystem.cpp) does the actual read-back
// once this command is reached during the normal backend dispatch.
struct visCaptureCommand_t {
	renderCommand_t		commandId;
	renderCommand_t *	next;
	bool				toNDI;						// true: capture + Vis_NDISendFrame; false: capture + write filePath
	bool				fixAlpha;					// only meaningful when !toNDI
	char				filePath[ MAX_OSPATH ];	// only meaningful when !toNDI
};
...
public:
	...
	virtual bool			CaptureRenderToMemory( byte * outBGRA, int outBufferSize, int & outWidth, int & outHeight );	// PRD M7
	virtual void			UnCrop();
	virtual bool			UploadImage( const char *imageName, const byte *data, int width, int height );

	virtual void			BeginOffscreenRender( const char * imageName, int width, int height );	// PRD M1 step 8
	virtual void			EndOffscreenRender();
	virtual void			EnqueueImGuiRender( void * drawDataClone );	// PRD M4
	virtual void			EnqueueCaptureToFile( const char * fileName, bool fixAlpha );	// PRD M6/M7
	virtual void			EnqueueCaptureToNDI();							// PRD M6/M7
	virtual void			EnqueueVisMilkShaderCompile( const char * vertexSource, const char * fragmentSource, const char * compFragmentSource, bool isMashB );	// PRD M3/M5
	virtual void			EnqueueVisMilkWarpDraw( const char * imageName, const visMilkWarpUniforms_t & uniforms );	// PRD M3
	virtual void			EnqueueVisMilkMashDraw( const char * imageName, const visMilkWarpUniforms_t & uniforms, float mashMix );	// PRD M5
...
public:
	...
	// PRD M1 step 8: non-NULL between a BeginOffscreenRender/EndOffscreenRender
	// pair -- the image subsequent 2D draws are being redirected into. Guards
	// against nested/mismatched calls; NULL means "drawing to the backbuffer
	// as normal" (the overwhelmingly common case -- every other renderer
	// caller is unaffected by this feature existing).
	idImage *				offscreenRenderImage;
...
extern idCVar r_forceAmbientDiffuse;		// draw each surface's diffusemap unlit via the ambient pass (map-flythrough preview)
extern idCVar r_showTrisHue;				// tint RB_ShowTris's wireframe color (map-flythrough preview); negative = plain white
extern idCVar r_showTrisBlendMode;			// override RB_ShowTris's wireframe blend mode (map-flythrough preview); negative = unchanged
...
void RB_VisFboBegin( const void *data );	// PRD M1 step 8
void RB_VisFboEnd( const void *data );
void RB_ImGuiRender( const void *data );	// PRD M4, implemented in neo/sys/win32/win_imgui.cpp
void RB_VisCapture( const void *data );	// PRD M6/M7, implemented in RenderSystem.cpp
void RB_VisMilkShaderCompile( const void *data );	// PRD M3
void RB_VisMilkWarpDraw( const void *data );		// PRD M3
void RB_VisMilkMashDraw( const void *data );		// PRD M5
int RB_GetMilkWarpCompileStatus();					// PRD M3: 0=none, 1=ok, -1=failed -- see tr_backend_draw.cpp
int RB_GetMilkMashCompileStatus();					// PRD M5: same, for the mash-up B side
